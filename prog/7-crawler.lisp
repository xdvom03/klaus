...
(defun crawl (seed queue-size page-count target)
    ...
    (dotimes (i page-count)
      ...
      (let* ((best-url (pick-from-queue queue))
             ...
             (links (mapcar #'remove-wikipedia-boilerplate
                            (remove-duplicates (remove-if-not #'(lambda (url) (print (concat "checking: " url)) (and (url-allowed? *crawler-name* url)
                                               ...
                                               :test #'equal))))
        (push best-url acc)
        (with-open-file (stream (concat *wanderbot-file* "-focused-found")
                                :direction :output :if-exists :append :if-does-not-exist :create)
          (print best-url stream))
        (setf queue (remove-nth 0 (mapcar #'(lambda (entry) (cons (car entry) (* 1 (cdr entry)))) ; taxing the old links
          ...
          (if (not (or (member link acc :test #'equal)
              ...
              (let* ((sorted-queue (sort (copy-seq queue) #'> :key #'cdr))
                ...
                (if (equal (url-text link)
                    ...
                    (progn
                      (with-open-file (stream (concat *wanderbot-file* "-focused-all")
                                              :direction :output :if-exists :append :if-does-not-exist :create)
                        (print link stream))
                      (print "score:")
                      (print score)
